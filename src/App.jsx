...
function App() {
  ...
  useEffect(() => {
    ...
    if (!user?.role) {
      setIsLoading(true);
      privateAxios
        .post("users/userByToken/", { access_token: token })
        .then((res) => {
  ...
  if (isFirefox) document.documentElement.style.fontSize = "14px";
    ...
    <>
      <AuthRedirectHandler>
        ...
        <Routes>
          ...
          <Route
            path="/course/:course_name/enroll/:lessonId/step/:num"
            element={
              <EnrollCourseProvider>
                {" "}
                <EnrollCourse />{" "}
              </EnrollCourseProvider>
            }
